...
def after_load_game_file(game_table: dict) -> dict:
    game_table.update(spec.game)
    return game_table

# called after the items.json file has been loaded, before any item loading or processing has occurred
# if you need access to the items after processing to add ids, etc., you should use the hooks in World.py
def after_load_item_file(item_table: list) -> list:
    item_table.extend(spec.items.values())
    return item_table
...
def after_load_location_file(location_table: list) -> list:
    location_table.extend(spec.locations.values())
    return location_table

# called after the locations.json file has been loaded, before any location loading or processing has occurred
# if you need access to the locations after processing to add ids, etc., you should use the hooks in World.py
def after_load_region_file(region_table: dict) -> dict:
    region_table.update(spec.regions)
    return region_table
...
def after_load_option_file(option_table: dict) -> dict:
    ...
    option_table["core"].update(spec.core_options)
    return option_table
